...
PowerShell の `System.Net.HttpListener` と `Markdig.dll` を組み合わせることで、複雑な Node.js や Python 実行環境の導入なしに動作します。

* **対象 OS**: Windows 11
* **対象 PowerShell バージョン**: Windows PowerShell 5.1 (.NET Framework 4.8)
* **.NET アセンブリ要件**: `Markdig.dll` は `.NET Framework 4.6.2` (net462) ビルド版を使用
...
```text
MyWiki/
├── Start-MarkdigWiki.ps1   <-- メインスクリプト (UTF-8 with BOM)
├── Start-MarkdigWiki.bat   <-- 起動用バッチファイル (UTF-8 without BOM)
├── lib/
│   └── Markdig.dll          <-- .NET Framework 4.6.2 ビルド版 Markdig.dll (net462)
├── tests/
│   └── Start-MarkdigWiki.Tests.ps1 <-- Pester ユニット/統合テストスクリプト
├── index.md                 <-- トップページ用
└── docs/
    └── 詳細仕様.md           <-- サブフォルダ内のページ
```
...
## 3. ファイルエンコーディング規約 (`AGENTS.md` 準拠)

| ファイル種別 | 拡張子 | 文字コード | 理由 |
| :--- | :--- | :--- | :--- |
| PowerShell スクリプト | `.ps1` | **UTF-8 with BOM (`EF BB BF`)** | Windows PowerShell 5.1 での日本語文字化け防止 |
| バッチファイル | `.bat` | **UTF-8 without BOM (No-BOM)** | `cmd.exe` の `・ｿ` 誤認識による `@echo` エラー防止 |
| マークダウン | `.md` | **UTF-8 (BOMあり/なし可)** | 標準テキスト |

---

## 4. メインスクリプト仕様 (`Start-MarkdigWiki.ps1`)

### 主な変更点・堅牢化対応
1. **Markdig API 正確化**: `[Markdig.MarkdownExtensions]::UseAdvancedExtensions($builder)` を使用（クラス名不一致バグの修正）。
2. **ディレクトリトラバーサル防止**: `[System.IO.Path]::GetFullPath` で検証し、`$wikiDir` 外へのパスアクセスに対して `403 Forbidden` を返却。
3. **XSS 対策**: 404 レンダリング時の `$rawPath` やサイドバー・HTML タイトル内の文字列を `[System.Net.WebUtility]::HtmlEncode()` でエスケープ。
4. **リソースリーク防止**: リクエスト処理全体を `try { ... } finally { $response.Close() }` で囲み、変換失敗時も HTTP レスポンスを確実に切断。
5. **パフォーマンス改善**: サイドバー生成時に `.git` や `lib` 等の不要ディレクトリを除外。
...
#  対応: PowerShell 5.1 / PowerShell 7+
...
$wikiDir     = [System.IO.Path]::GetFullPath($PSScriptRoot)
$fullWikiDir = $wikiDir.TrimEnd('\', '/') + [System.IO.Path]::DirectorySeparatorChar
$libDir      = Join-Path $wikiDir "lib"
...
function Get-SidebarHtml {
    ...
    $mdFiles = Get-ChildItem -Path $wikiDir -Recurse -Filter "*.md" | 
        Where-Object { $_.FullName -notmatch '[\\/]\.(git|lib)[\\/]' } |
        Sort-Object FullName

    $html = "<ul>`n"
    foreach ($file in $mdFiles) {
        $relPath = $file.FullName.Substring($wikiDir.Length).TrimStart("\", "/")
        $webPath = "/" + ([Uri]::EscapeUriString($relPath -replace "\\", "/"))
...
<head>
<meta charset="UTF-8">
<title>${pageTitle} - Local Wiki</title>
<style>
...
## 5. ダブルクリック用バッチ (`Start-MarkdigWiki.bat`)
...
`Invoke-ScriptAnalyzer` を用いて、スクリプトの静的解析チェックを実施します。

### ② Pester ユニット / 統合テスト
`tests/Start-MarkdigWiki.Tests.ps1` を作成し、以下を自動テストで検証します：
1. **アセンブリロード検証**: `Markdig.dll` のロードおよび Markdown パイプライン構築が正常に行われるか。
2. **パス安全性の検証**: ディレクトリトラバーサル攻撃パス（`../`）が正しく拒否（403 等）されるか。
3. **HTML エスケープ検証**: 404 エラー時のパス文字列が正しく `HtmlEncode` されるか。
4. **サイドバー生成**: ファイル一覧が正しくリスト化され、除外フォルダ（`.git`）が含まれないか。
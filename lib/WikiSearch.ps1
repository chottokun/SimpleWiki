    ...
    try {
        ...
        if ((Test-Path $targetDir) -and (Test-Path -LiteralPath $cacheFilePath)) {
            ...
            if ($cacheItem) {
                ...
                # キャッシュファイル作成後に更新された .md ファイルが存在するかチェック
                $newMdFiles = Get-ChildItem -Path $targetDir -Recurse -Filter "*.md" -ErrorAction SilentlyContinue |
                    Where-Object { $_.FullName -notmatch '[\\/]\.(git|lib|tests|dist|\.cache)[\\/]' -and $_.LastWriteTime -gt $cacheFileWriteTime }
                if ($newMdFiles.Count -gt 0) {
                    return $false
                }
            }
        }

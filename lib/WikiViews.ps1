...
function Get-SettingsViewHtml {
    ...
    $cachedCount = $script:WikiIndex.Count
    $lastScanStr = if ($script:WikiIndexLastScan -gt [DateTime]::MinValue) { $script:WikiIndexLastScan.ToString("yyyy-MM-dd HH:mm:ss") } else { "未実行" }
    ...
    <form id="settingsForm" onsubmit="saveSettings(event)" style="display: flex; flex-direction: column; gap: 20px;">
        <div class="okf-card">
            ...
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #eaecef; font-size: 13px; color: #586069; display: flex; justify-content: space-between; align-items: center;">
                ...
                <button type="button" onclick="rebuildIndexNow()" style="padding: 6px 12px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">
                    🔄 今すぐインデックス再生成
                </button>
            </div>
        </div>
...
function rebuildIndexNow() {
    ...
    .then(function(data) {
        if (data.success) {
            ...
            showToast('❌ 再生成失敗: ' + (data.message || 'エラーが発生しました'), true);
        }
    })
    .catch(function(err) {
        showToast('❌ 再生成中に通信エラーが発生しました。', true);
    });
}
</script>
"@

...
Describe 'Index Cache and Settings View Tests' {
    ...
    It "Save-WikiIndexCache and Load-WikiIndexCache cycle works when useCache is enabled" {
        ...
        try {
            ...
            $cachePath = Get-WikiCachePath -TargetWikiDir $tempDir -TargetScriptDir $tempScriptDir
            if (Test-Path $cachePath) { Remove-Item -Path $cachePath -Force -ErrorAction SilentlyContinue }
            Remove-Item -Path $testMd -Force -ErrorAction SilentlyContinue
            $script:WikiIndex = @()
...
"@
    ...
    Context "PR #14 & #16 & #24 & #29 & #30 & #31: Trees, Exporters, and PSObject Conversion" {
        ...
        It "Build-ServerFileTreeNode and Test-ServerNodeHasActiveFile build recursive trees accurately" {
            ...
            $mdFiles = @(
                [PSCustomObject]@{ RelPath = "index.md"; Title = "index" },
                [PSCustomObject]@{ RelPath = "docs/guide/start.md"; Title = "start" }
            )

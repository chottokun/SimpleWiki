...
$script:libDll = Join-Path $script:projectRoot (Join-Path "lib" "Markdig.dll")
$libDll = $script:libDll

Describe 'Markdig Assembly and Pipeline Tests' {
    It "Markdig.dll exists in lib directory" {
        $lib = Join-Path $script:projectRoot (Join-Path "lib" "Markdig.dll")
        [bool](Test-Path $lib) | Should -Be $true
    }
...
Describe "Static HTML Export Tests (Export-MarkdigWiki.ps1)" {
    ...
    It "Exports static HTML files to target directory" {
        ...
        (Test-Path (Join-Path $testExportDir (Join-Path "docs" "詳細仕様.html"))) | Should -Be $true
    }
    ...
    It "Generates relative URI links for subfolder pages without leading slash" {
        $subHtmlPath    = Join-Path $testExportDir (Join-Path "docs" "詳細仕様.html")
...
Describe 'Editor Settings and Read-Only Guard Tests' {
    ...
    It "/api/config saves editor enabled and maxBackups settings safely" {
        ...
        $proc = $pwshPath = if (Get-Command pwsh -ErrorAction SilentlyContinue) { (Get-Command pwsh).Source } else { "powershell" }
        $proc = Start-Process $pwshPath -ArgumentList "-File", (Join-Path $tempIsolatedDir "Start-MarkdigWiki.ps1"), "-RootFolder", (Join-Path $tempIsolatedDir "markdown_sample"), "-Port", $port -PassThru
...
Describe "UI Shutdown and Brand Title Customization Tests" {
    ...
    It "/api/config handles OrderedDictionary and saves config.json without errors" {
        ...
        $port = 8094
        $pwshBin = if (Get-Command pwsh -ErrorAction SilentlyContinue) { (Get-Command pwsh).Source } else { "powershell" }
        $proc = Start-Process $pwshBin -ArgumentList "-File", (Join-Path $tempIsolatedDir "Start-MarkdigWiki.ps1"), "-RootFolder", (Join-Path $tempIsolatedDir "markdown_sample"), "-Port", $port -PassThru
        Start-Sleep -Seconds 3
        try {
            $payload = @{
                search = @{

...
Describe 'Critical Edge Case and Security Tests' {
    ...
    It 'Executes Unblock-File safely on lib DLLs without throwing exceptions' {
        ...
        if ($IsWindows -or $env:OS -eq "Windows_NT") { { Get-ChildItem -Path $libPath -Filter "*.dll" | Unblock-File -ErrorAction SilentlyContinue } | Should -Not -Throw }
    }
}
...
Describe 'OKF LLM RAG Security and Encryption Tests' {
    ...
    It 'Encrypts and decrypts API key with Windows DPAPI (DPAPI: prefix)' {
        if (-not ($IsWindows -or $env:OS -eq "Windows_NT")) { Set-ItResult -Skipped -Because "DPAPI is Windows-only"; return }
...
Describe 'Convert-PSObjectToOrdered Utility Tests' {
    BeforeAll {
        $projectRoot = if ($script:projectRoot) { $script:projectRoot } else { $PWD.Path }
        $serverScript = Join-Path $projectRoot "Start-MarkdigWiki.ps1"
        . $serverScript -DotSourceOnly
    }

    It 'Returns null when input is null' {
        $result = Convert-PSObjectToOrdered $null
        $result | Should -Be $null
    }

    It 'Returns primitive values unchanged' {
        (Convert-PSObjectToOrdered "hello") | Should -Be "hello"
        (Convert-PSObjectToOrdered 123) | Should -Be 123
        (Convert-PSObjectToOrdered $true) | Should -Be $true
        (Convert-PSObjectToOrdered 3.14) | Should -Be 3.14
    }

    It 'Returns IDictionary / Hashtable as is' {
        $hash = @{ key1 = "val1"; key2 = 2 }
        $result = Convert-PSObjectToOrdered $hash
        $result -is [System.Collections.IDictionary] | Should -Be $true
        $result["key1"] | Should -Be "val1"
        $result["key2"] | Should -Be 2

        $orderedHash = [ordered]@{ a = 1; b = 2 }
        $resultOrdered = Convert-PSObjectToOrdered $orderedHash
        $resultOrdered -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $resultOrdered["a"] | Should -Be 1
    }

    It 'Converts flat PSCustomObject into OrderedDictionary preserving property order' {
        $psobj = [PSCustomObject]@{
            Zebra = 100
            Apple = "fruit"
            Banana = $true
        }
        $result = Convert-PSObjectToOrdered $psobj
        $result -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $result.Count | Should -Be 3

        $keys = @($result.Keys)
        $keys[0] | Should -Be "Zebra"
        $keys[1] | Should -Be "Apple"
        $keys[2] | Should -Be "Banana"

        $result["Zebra"] | Should -Be 100
        $result["Apple"] | Should -Be "fruit"
        $result["Banana"] | Should -Be $true
    }

    It 'Converts empty PSCustomObject into empty OrderedDictionary' {
        $psobj = [PSCustomObject]@{}
        $result = Convert-PSObjectToOrdered $psobj
        $result -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
    ...
    It 'Recursively converts nested PSCustomObject instances into OrderedDictionaries' {
        $psobj = [PSCustomObject]@{
            OuterKey = "outer"
        ...
        $result = Convert-PSObjectToOrdered $psobj
        $result -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $result["OuterKey"] | Should -Be "outer"

        $nested = $result["NestedObj"]
        $nested -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $nested["InnerKey"] | Should -Be "inner"

        $deep = $nested["DeepObj"]
        $deep -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $deep["Value"] | Should -Be 42
    }

    It 'Recursively converts arrays containing PSCustomObject instances' {
        $psobjArray = @(
            [PSCustomObject]@{ Item = "First"; Code = 1 },
            [PSCustomObject]@{ Item = "Second"; Code = 2 }
        )
        $result = Convert-PSObjectToOrdered $psobjArray
        $result -is [System.Array] | Should -Be $true
        $result.Count | Should -Be 2

        $result[0] -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $result[0]["Item"] | Should -Be "First"
        $result[0]["Code"] | Should -Be 1

        $result[1] -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $result[1]["Item"] | Should -Be "Second"
        $result[1]["Code"] | Should -Be 2
    }

    It 'Handles complex mixed structure with nested arrays, custom objects, hashtables, primitives, and nulls' {
        $complexObj = [PSCustomObject]@{
            ConfigName = "WikiServer"
            Port = 8080
            NullVal = $null
            Tags = @("tag1", "tag2")
            SubModules = @(
        ...
        $result = Convert-PSObjectToOrdered $complexObj
        $result -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $result["ConfigName"] | Should -Be "WikiServer"
        $result["Port"] | Should -Be 8080
        $result["NullVal"] | Should -Be $null
        $result["Tags"] -is [System.Array] | Should -Be $true
        $result["Tags"][0] | Should -Be "tag1"

        $subModules = $result["SubModules"]
        $subModules -is [System.Array] | Should -Be $true
        $subModules[0] -is [System.Collections.Specialized.OrderedDictionary] | Should -Be $true
        $subModules[0]["Name"] | Should -Be "Search"
        $subModules[0]["Enabled"] | Should -Be $true
        $subModules[0]["Params"] -is [System.Collections.IDictionary] | Should -Be $true

        $subModules[1]["Name"] | Should -Be "Rag"
        $subModules[1]["Enabled"] | Should -Be $false
        $subModules[1]["Params"] | Should -Be $null
    }
}

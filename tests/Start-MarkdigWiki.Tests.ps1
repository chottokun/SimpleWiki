...
Describe "Static HTML Export Tests (Export-MarkdigWiki.ps1)" {
    ...
    It "Converts .md hyperlinks to .html in exported files" {
        ...
        $htmlContent | Should -Match "概要.html"
    ...
    It "Generates relative URI links for subfolder pages without leading slash" {
        ...
        $subHtmlContent | Should -Match "\.\./index.html"
        $subHtmlContent | Should -Match "概要.html"
    }
...
Describe 'Index Cache and Settings View Tests' {
    BeforeAll {
        ...
        $projectRoot = (Get-Item $testScriptDir).Parent.FullName
        $sampleDir = Join-Path $projectRoot "markdown_sample"
    }

    It "Get-WikiCachePath produces valid cross-platform cache file path under scriptDir with folder hash" {
        $cachePath = Get-WikiCachePath -TargetWikiDir $sampleDir
    ...
    It "Save-WikiIndexCache and Load-WikiIndexCache cycle works when useCache is enabled" {
        ...
        try {
            ...
            (Get-Item $tempDir).LastWriteTime = (Get-Date).AddMinutes(1)
            $script:WikiIndex = @()
    ...
    It "Clear-AllWikiCaches removes all index-cache files and resets in-memory cache" {
        $tempScriptDir = Join-Path ([System.IO.Path]::GetTempPath()) "SimpleWiki_TestClearCacheDir"
        if (Test-Path $tempScriptDir) { Remove-Item -Path $tempScriptDir -Recurse -Force -ErrorAction SilentlyContinue }
        $null = New-Item -ItemType Directory -Path $tempScriptDir
        $cacheDir = Join-Path $tempScriptDir ".cache"
        ...
        try {
            $clearResult = Clear-AllWikiCaches -TargetScriptDir $tempScriptDir
    ...
    It "Get-WikiStatusPath returns valid cross-process status file path" {
        $statusPath = Get-WikiStatusPath -TargetWikiDir $projectRoot -TargetScriptDir $projectRoot
...
Describe "UI Shutdown and Brand Title Customization Tests" {
    ...
    It "/api/config handles OrderedDictionary and saves config.json without errors" {
        ...
        $port = 8094
        $samplePath = Join-Path $tempIsolatedDir "markdown_sample"
        $scriptPath = Join-Path $tempIsolatedDir "Start-MarkdigWiki.ps1"
        $proc = Start-Process -FilePath "pwsh" -ArgumentList "-File", "`"$scriptPath`"", "-RootFolder", "`"$samplePath`"", "-Port", "$port" -PassThru -NoNewWindow
        ...
        try {
            ...
            if ($IsWindows -or $env:OS -eq "Windows_NT") {
                ...
                $savedCfg.rag.apiKey | Should -Match "^ENC:"
            }
...
Describe 'Render-DocList HTML Rendering Tests' {
    BeforeAll {
        . (Join-Path $projectRoot "Start-MarkdigWiki.ps1") -DotSourceOnly
    }

    It "Returns empty message paragraph when docArray is empty or null" {
        $msg = "ドキュメントはありません"
        $htmlNull = Render-DocList -docArray $null -emptyMsg $msg
        $htmlNull | Should -Be "<p class='empty-msg'>$msg</p>"

        $htmlEmpty = Render-DocList -docArray @() -emptyMsg $msg
        $htmlEmpty | Should -Be "<p class='empty-msg'>$msg</p>"
    }

    It "Renders single document correctly with formatted date and escaped URI" {
        $item = [PSCustomObject]@{
            Title       = "運用マニュアル"
            RelPath     = "docs\manual.md"
            LastUpdated = (Get-Date "2026-05-15")
        }
        $html = Render-DocList -docArray @($item) -emptyMsg "なし"
        $html | Should -Match "<ul>"
        $html | Should -Match "<li><a href='/docs/manual.md'>運用マニュアル</a> <span class='muted'>\(2026-05-15\)</span></li>"
        $html | Should -Match "</ul>"
    }

    It "Renders multiple documents correctly" {
        $items = @(
            [PSCustomObject]@{
                Title       = "Doc 1"
                RelPath     = "docs/doc1.md"
                LastUpdated = (Get-Date "2026-01-01")
            },
            [PSCustomObject]@{
                Title       = "Doc 2"
                RelPath     = "docs/doc2.md"
                LastUpdated = (Get-Date "2026-02-01")
            }
        )
        $html = Render-DocList -docArray $items -emptyMsg "なし"
        $html | Should -Match "<li><a href='/docs/doc1.md'>Doc 1</a> <span class='muted'>\(2026-01-01\)</span></li>"
        $html | Should -Match "<li><a href='/docs/doc2.md'>Doc 2</a> <span class='muted'>\(2026-02-01\)</span></li>"
    }

    It "HTML encodes special characters in document titles" {
        $item = [PSCustomObject]@{
            Title       = "<script>alert('xss')</script> & Test"
            RelPath     = "docs/xss.md"
            LastUpdated = (Get-Date "2026-03-01")
        }
        $html = Render-DocList -docArray @($item) -emptyMsg "なし"
    ...
    It "Handles string LastUpdated gracefully" {
        $item = [PSCustomObject]@{
            Title       = "String Date Doc"
            RelPath     = "docs/strdate.md"
            LastUpdated = "2026-04-01"
        }
        $html = Render-DocList -docArray @($item) -emptyMsg "なし"
        $html | Should -Match "\(2026-04-01\)"
    }

    It "Get-MaintenanceViewHtml integrates Render-DocList for stale, draft, and deprecated docs" {
        $sampleDir = Join-Path $projectRoot "markdown_sample"
        Build-WikiIndex -TargetWikiDir $sampleDir -ForceRefresh | Out-Null
        $maintHtml = Get-MaintenanceViewHtml -Lang "ja"
        $maintHtml | Should -Match "品質"
        $maintHtml | Should -Match "maint-section"
    }
}

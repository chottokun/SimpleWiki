...
if (-not $PSScriptRoot) { $PSScriptRoot = Join-Path (Get-Location).Path "tests" }
$script:projectRoot = (Resolve-Path "$PSScriptRoot/..").Path
$projectRoot        = $script:projectRoot
$script:libDll      = Join-Path $script:projectRoot "lib/Markdig.dll"
$libDll             = $script:libDll
...
"@
    ...
    It "Get-OkfCardHtml combines top bar and footer card HTML with metadata and edit button" {
        $meta = [PSCustomObject]@{
            Title       = "Card Component Spec"
            Description = "Specifies HTML card rendering"
            Author      = "Dev Lead"
            Domain      = "specs/ui"
            Status      = "active"
            Tags        = @("UI", "Card")
            LastUpdated = (Get-Date "2026-09-10")
        }

        $cardHtml = Get-OkfCardHtml -Meta $meta -RelPath "specs/ui/card.md" -Lang "ja"
        $cardHtml | Should -Match 'class="okf-top-bar"'
        $cardHtml | Should -Match 'class="okf-footer-card"'
        $cardHtml | Should -Match '📁 specs/ui'
        $cardHtml | Should -Match "data-relpath='specs/ui/card.md'"
        $cardHtml | Should -Match '✏️ 編集'
        $cardHtml | Should -Match 'Specifies HTML card rendering'
        $cardHtml | Should -Match 'Dev Lead'
        $cardHtml | Should -Match '2026-09-10'
    }

    It "Get-OkfCardHtml supports multi-language localization (en)" {
        $meta = [PSCustomObject]@{
            Title       = "English Card Doc"
            Description = "English metadata card test"
            Author      = "Alice"
            Domain      = "docs/en"
            Status      = "active"
            LastUpdated = (Get-Date "2026-01-15")
        }

        $cardHtmlEn = Get-OkfCardHtml -Meta $meta -RelPath "docs/en/card.md" -Lang "en"
        $cardHtmlEn | Should -Match 'Document Metadata \(OKF\)'
        $cardHtmlEn | Should -Match 'Author:'
        $cardHtmlEn | Should -Match '✏️ Edit'
        $cardHtmlEn | Should -Match 'Last Updated:'
    }
}

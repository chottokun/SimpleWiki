...
Describe 'Editor Settings and Read-Only Guard Tests' {
    ...
    It "/api/config saves editor enabled and maxBackups settings safely" {
        ...
        $proc = Start-Process $psExe -ArgumentList "-NoProfile", "-File", (Join-Path $tempIsolatedDir "Start-MarkdigWiki.ps1"), "-RootFolder", (Join-Path $tempIsolatedDir "markdown_sample"), "-Port", $port -PassThru

...
Describe "Unit and Integration Tests for Export Sidebar Functions (Get-ExportSidebarHtml)" {
    BeforeAll {
        $testProjectRoot = if ($projectRoot) { $projectRoot } else { (Resolve-Path (Join-Path $PSScriptRoot "..")).Path }
        $exportScript = Join-Path $testProjectRoot "Export-MarkdigWiki.ps1"
        $tokens = $null
        $errors = $null
        $ast = [System.Management.Automation.Language.Parser]::ParseFile($exportScript, [ref]$tokens, [ref]$errors)

        # Dot-source function definitions from Export-MarkdigWiki.ps1 without running top-level script body
        $functionsToSource = @("Build-FileTreeNode", "Test-ExportNodeHasActiveFile", "Render-ExportFolderTreeHtml", "Get-ExportSidebarHtml")
        foreach ($funcName in $functionsToSource) {
    ...
    It "Build-FileTreeNode correctly builds node tree with files and subfolders" {
        $wikiDir = "/app/wiki"
        $files = @(
            [PSCustomObject]@{ FullName = "$wikiDir/index.md"; BaseName = "index" },
            [PSCustomObject]@{ FullName = "$wikiDir/docs/guide.md"; BaseName = "guide" },
            [PSCustomObject]@{ FullName = "$wikiDir/docs/api/spec.md"; BaseName = "spec" }
        )

        $tree = Build-FileTreeNode -allMdFiles $files -wikiDir $wikiDir
        $tree.Files.Count | Should -Be 1
        $tree.Files[0].BaseName | Should -Be "index"
        $tree.SubFolders.Contains("docs") | Should -Be $true
        $tree.SubFolders["docs"].Files[0].BaseName | Should -Be "guide"
        $tree.SubFolders["docs"].SubFolders.Contains("api") | Should -Be $true
        $tree.SubFolders["docs"].SubFolders["api"].Files[0].BaseName | Should -Be "spec"
    }

    It "Test-ExportNodeHasActiveFile detects active file in node or nested subfolders" {
        $wikiDir = "/app/wiki"
        $activeFile = [PSCustomObject]@{ FullName = "$wikiDir/docs/sub/doc.md"; BaseName = "doc" }
        $otherFile  = [PSCustomObject]@{ FullName = "$wikiDir/docs/other.md"; BaseName = "other" }
        ...
        $rootNode = [PSCustomObject]@{
            Files      = [System.Collections.Generic.List[PSObject]]@($otherFile)
            SubFolders = [ordered]@{ "sub" = $subNode }
        }

        (Test-ExportNodeHasActiveFile -node $rootNode -currentFile $activeFile) | Should -Be $true
        (Test-ExportNodeHasActiveFile -node $subNode -currentFile $otherFile) | Should -Be $false
    }

    It "Render-ExportFolderTreeHtml sorts files with index first, README second, and applies open attribute" {
        $wikiDir = "/app/wiki"
        $fileIndex  = [PSCustomObject]@{ FullName = "$wikiDir/index.md"; BaseName = "index" }
        $fileReadme = [PSCustomObject]@{ FullName = "$wikiDir/README.md"; BaseName = "README" }
        $fileZeta   = [PSCustomObject]@{ FullName = "$wikiDir/zeta.md"; BaseName = "zeta" }
        $fileAlpha  = [PSCustomObject]@{ FullName = "$wikiDir/alpha.md"; BaseName = "alpha" }

        $node = [PSCustomObject]@{
            Files      = [System.Collections.Generic.List[PSObject]]@($fileZeta, $fileAlpha, $fileReadme, $fileIndex)
            SubFolders = [ordered]@{}
        }

        $currentUri = New-Object System.Uri("$wikiDir/index.html")
        $html = Render-ExportFolderTreeHtml -node $node -currentFile $fileIndex -currentUri $currentUri

        $idxPos = $html.IndexOf("📄 index")
        $readmePos = $html.IndexOf("📄 README")
        $alphaPos = $html.IndexOf("📄 alpha")
        $zetaPos = $html.IndexOf("📄 zeta")

        ($idxPos -lt $readmePos) | Should -Be $true
        ($readmePos -lt $alphaPos) | Should -Be $true
        ($alphaPos -lt $zetaPos) | Should -Be $true
        $html | Should -Match "href='' class='active'"
    }

    It "Get-ExportSidebarHtml produces formatted HTML with active file class and HTML-encoded titles" {
        $wikiDir = "/app/wiki"
        $file1 = [PSCustomObject]@{ FullName = "$wikiDir/<Test & Page>.md"; BaseName = "<Test & Page>" }
        $file2 = [PSCustomObject]@{ FullName = "$wikiDir/docs/sub/spec.md"; BaseName = "spec" }

        $allFiles = @($file1, $file2)
        $html = Get-ExportSidebarHtml -currentFile $file2 -allMdFiles $allFiles -wikiDir $wikiDir

        $html | Should -Match "📄 &lt;Test &amp; Page&gt;"

...
Describe "Static HTML Export Tests (Export-MarkdigWiki.ps1)" {
    ...
    It "Exports static HTML files to target directory" {
        ...
        $sampleDir    = Join-Path $script:projectRoot "markdown_sample"
...
Describe 'Search Query NOT Syntax Tests' {
    ...
    It "TC-NOT-08: Agentic tool Invoke-ToolSearchOkf respects NOT syntax and excludes target" {
        $toolRes = Invoke-ToolSearchOkf -Query "API -Python" -WikiDir $script:projectRoot
...
Describe 'OKF LLM RAG Security and Encryption Tests' {
    ...
    It 'New-ActivationCode.ps1 CLI script supports both Machine-Bound and Legacy portable modes' {
        $cliScript = Join-Path $script:projectRoot "New-ActivationCode.ps1"
...
Describe 'Agentic RAG and OKF Tools Tests' {
    ...
    It "Invoke-AgenticRagChat supports -Stream, -OnThinkingCallback, and -OnChunkReceived parameters" {
        ...
        $chunkList = [System.Collections.Generic.List[string]]::new()
        $res = Invoke-AgenticRagChat -ApiUrl "http://invalid-endpoint-xyz-999" -ApiKey "key" -Model "model" -UserMessage "テスト質問" -WikiDir $script:projectRoot -MaxTurns 1 -TimeoutSec 1 -Stream -OnThinkingCallback { param($m) $thinkList.Add($m) } -OnChunkReceived { param($c) $chunkList.Add($c) }
        ($null -ne $res) | Should -Be $true
        $res.answer | Should -Not -BeNullOrEmpty
    }
}
...
Describe 'Editor Settings and Read-Only Guard Tests' {
    ...
    It "Get-SettingsViewData includes EditorEnabledChecked and EditorMaxBackups fields for ja and en" {
        ...
        $dataJa.EditorMaxBackups | Should -BeGreaterThan -1
    ...
    It "/api/config saves editor enabled and maxBackups settings safely" {
        ...
        Copy-Item -Path (Join-Path $script:projectRoot "Start-MarkdigWiki.ps1") -Destination $tempIsolatedDir -Force
        Copy-Item -Path (Join-Path $script:projectRoot "lib") -Destination (Join-Path $tempIsolatedDir "lib") -Recurse -Force
        Copy-Item -Path (Join-Path $script:projectRoot "markdown_sample") -Destination (Join-Path $tempIsolatedDir "markdown_sample") -Recurse -Force
        ...
        $port = 8095
        $proc = Start-Process "pwsh" -ArgumentList "-File", (Join-Path $tempIsolatedDir "Start-MarkdigWiki.ps1"), "-RootFolder", (Join-Path $tempIsolatedDir "markdown_sample"), "-Port", $port -PassThru
...
Describe 'Index Cache and Settings View Tests' {
    ...
    It "Get-WikiCachePath produces valid cross-platform cache file path under scriptDir with folder hash" {
        ...
        # キャッシュの親ディレクトリがスクリプト配置元 ($scriptDir / $script:projectRoot) 配下であることを検証
        $cachePath.StartsWith($script:projectRoot, [System.StringComparison]::OrdinalIgnoreCase) | Should -Be $true
    }
    ...
    It "Start-MarkdigWiki.ps1 includes /api/indexing-status endpoint" {
        $scriptContent = Get-Content -Path (Join-Path $script:projectRoot "Start-MarkdigWiki.ps1") -Raw -Encoding UTF8
...
Describe "Multi-Language (i18n) & Localization Tests" {
    ...
    It "Dynamically merges external i18n.json dictionary entries" {
        $tempI18nFile = Join-Path $script:projectRoot "i18n.json"
...
Describe "UI Shutdown and Brand Title Customization Tests" {
    ...
    It "Start-MarkdigWiki.ps1 includes /api/shutdown endpoint and brand_title placeholder" {
        $scriptContent = [System.IO.File]::ReadAllText((Join-Path $script:projectRoot "Start-MarkdigWiki.ps1"), [System.Text.Encoding]::UTF8)
...
Describe "Repository Code Quality, Syntax & Character Encoding Validation Tests" {
    It "All PowerShell script files parse successfully without AST syntax errors" {
        $psFiles = Get-ChildItem -Path $script:projectRoot -Recurse -Include "*.ps1", "*.psm1", "*.psd1" |
    ...
    It "All Batch files (.bat) are strictly encoded as UTF-8 without BOM (No-BOM)" {
        $batFiles = Get-ChildItem -Path $script:projectRoot -Recurse -Include "*.bat" |
            Where-Object { $_.FullName -notmatch '[\\/]\.(git|cache)[\\/]' }
...
Describe "PR Consolidation & Unit Test Coverage Suite" {
    BeforeAll {
        ...
        $exportScriptPath = Join-Path $script:projectRoot "Export-MarkdigWiki.ps1"
    ...
    Context "PR #11 & #18: WikiMetadata Helper Functions" {
        It "Get-WikiDir resolves default and specified paths correctly" {
            $resolved = Get-WikiDir -RootFolder "" -TargetScriptDir $script:projectRoot
            (Test-Path $resolved) | Should -Be $true

            $resolvedCustom = Get-WikiDir -RootFolder $script:projectRoot -TargetScriptDir $script:projectRoot
            $resolvedCustom | Should -Be $script:projectRoot
        }

...
| **`/stella`** | 3D ステラビュー（Z軸時間深度・銀河マップ・関連ノード星座リンク・視点プリセット） | HTML (200 OK) |
...
| **`/api/shutdown`** | サーバープロセスの安全な終了要求 API | JSON (application/json) |
...
```mermaid
flowchart TD
    Start["開始"] --> Draft["新規作成 (下書き)"]
    Draft -->|"レビュー完了・正式公開"| Stable["正式公開 (安定版)"]
    Stable -->|"軽微修正・追記"| Stable
    Stable -->|"非推奨化 (旧版化)"| Deprecated["非推奨化 (旧版)"]
    Deprecated --> End["廃棄・アーカイブ"]
```
...
## ⚙️ 静的 HTML エキスポート (`Export-MarkdigWiki.ps1`) オプション仕様

| パラメータ | 型 / 選択肢 | デフォルト | 機能解説 |
| :--- | :--- | :---: | :--- |
| **`-Theme`** | `Auto` / `Light` / `Dark` | `Auto` | CSS 変数（`:root`）を用いたテーマ設定。`Auto` は OS のダークモード設定（`prefers-color-scheme: dark`）に自動追従。 |
| **`-PreserveCodeBlockLinks`** | switch | `$false` | コードスニペット（`<pre><code>`）内の `.md` や `href=` リンクを一時退避し、誤変換・破壊から保護。 |
| **`-DisableRawHtml`** | switch | `$false` | Markdig パイプラインの生 HTML 透過を遮断（`.DisableHtml()`）。信頼できない外部 Markdown のスクリプト注入を防止。 |
| **`-TemplatePath`** | string | `""` | 外部カスタム HTML テンプレートファイルを指定可能。 |
| **`-SingleFile`** | switch | `$false` | 全ドキュメントを単一の `index.html` に集約し、アンカーナビゲーションで閲覧する SPA モード。 |
| **`-EmbedImages`** | switch | `$false` | 画像を Base64 Data URI に変換して HTML 内部に埋め込み。`-SingleFile` 指定時は自動有効。 |
| **`-Language`** | `ja` / `en` | `ja` | エキスポート UI、メタデータカード、および目次の出力言語。 |

...
- 🔍 **豊富な動的ナビゲーションビュー ＆ 3D ステラビュー**:
  - 最近の更新 (`/recent`) | タグ目録 (`/tags`) | 品質ダッシュボード (`/maintenance`) | 著者一覧 (`/authors`) | 全文検索 (`/search`) | 3D ステラ (`/stella`) | システム設定 (`/settings`)
- 🌌 **3D ステラビュー (`/stella`)**: 時間軸（Z軸深度）とドキュメント間の関連性（共通タグ・関連文書リンク）を 3 次元宇宙空間として視覚化。3D銀河・意味マップ・タイムラインの視点プリセットに対応。
